...
where
    T: FromRequest,
    F: Fn(ServiceRequest, Option<T>, Option<Error>) -> O,
...
where
    F: Fn(ServiceRequest, Option<basic::BasicAuth>, Option<Error>) -> O,
    O: Future<Output = Result<ServiceRequest, (Error, ServiceRequest)>>,
{
...
where
    F: Fn(ServiceRequest, Option<bearer::BearerAuth>, Option<Error>) -> O,
    O: Future<Output = Result<ServiceRequest, (Error, ServiceRequest)>>,
{
...
where
    ...
    S::Future: 'static,
    F: Fn(ServiceRequest, Option<T>, Option<Error>) -> O + 'static,
    ...
    B: MessageBody + 'static,
{
    ...
    fn call(&self, req: ServiceRequest) -> Self::Future {
        ...
        Box::pin(async move {
            let (req, credentials, err) = match Extract::<T>::new(req).await {
                Ok((req, credentials)) => (req, Some(credentials), None),
                Err((err, req)) => (req, None, Some(err)),
            };

            let req = match process_fn(req, credentials, err).await {
                Ok(req) => req,
                Err((err, req)) => {
